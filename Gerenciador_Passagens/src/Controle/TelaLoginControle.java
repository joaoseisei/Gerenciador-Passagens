...
/**
 * A classe telaLoginControle é responsável por gerenciar o acesso as telas
 * "TelaAdmin" e "TelaUsuario', através do controle de Admin e Usuario no banco
 * de dados "memoria".
 * 
 * @author joaoseisei
 * @since 2023
...
//CONSTRUTOR
	/**
	 * Esse construtor é responsável por vincular a TelaLogin e a Memoria na classe,
...
//-------------------------VERIFICACAO---------------------------
	/**
	 * Esse método é responsável por verificar se existe um Usuario com o mesmo
	 * nome.
	 * 
	 * @param nome Nome a ser verificado.
	 * 
	 * @return Retorna true se existe um usuário com o mesmo nome e false se o nome
	 *         está disponivel.
	 */
...
//----------------------CRIACAO DE CONTA------------------------
	/**
	 * O método addConta é responsável por adicionar uma conta no banco de dados
	 * "Memoria", essa função utiliza somente os getters e setters para adicionar na
	 * conta.
	 * 
	 * @param tipo  Tipo de conta, se for true é administrativa e se for false é de
	 *              usuário.
	...
	/**
	 * O método criar conta é responsável por vincular o método addCOnta com os
	 * valores dos inputs, para isso ela filtra e analisa os dados recebidos não
	 * permitindo criar conta caso os inputs sejam nulos ou as senhas forem
	 * diferentes, se for diferente disso ele ira ver se já existe um usuário com o
	 * mesmo nome e se não tiver ele cadastra utilizando a função addConta.
	 * 
	 * @param nome      Nome do usuário.
	 * @param senha     Senha do Usuário.
	 * @param novaSenha Confirmação de senha.
	 * @param tipo      Tipo, se for true é cadastra uma conta administrativa e se
	 *                  for falso cadastra uma conta de usuário.
	 */
	public void criarConta(String nome, String senha, String novaSenha, boolean tipo) {
		if (nome == null || senha == null || novaSenha == null || nome.trim().isEmpty() || senha.trim().isEmpty()
			...
			if ((verificarUserRegistro(nome) == false || memoria.getListaUsuario().isEmpty()) && tipo == false) {
				addConta(tipo, nome, senha);
			} else if ((verificarAdminRegistro(nome) == false || memoria.getListaAdmin().isEmpty()) && tipo == true) {
				addConta(telaLogin.getTipo(), telaLogin.getNome(), telaLogin.getSenha());
			} else {
				JOptionPane.showMessageDialog(null, "Já existe um usuario cadastrado com esse nome", "ERRO",
...
//----------------------FAZER LOGIN------------------------
	/**
	 * Esse método responsável por verificar se existe um usuário existente com o
	 * mesmo nome, mesma senha e mesmo tipo no banco de dados.
	 * 
	 * @param nome  Nome a ser analisado.
	 * @param senha Senha a ser analisada.
	 * @param tipo  Tipo de conta, se for true é administrativa e se for false e
	 *              usuário.
	 * 
	 * @return Se existir um usuário ou admin correspondente o retorno será
	 *         "UsuarioLogado" ou "AdminLogado" respectivamente, se não tiver nenhum
	 *         correspondente o resultado será "NenhumaContaEncontada".
	 */
	...
	/**
	 * Esse método é responsável por gerenciar o acesso a outras telas, através do
	 * metódo verificacaoLogin.
	 * 
	 * @param nome  Nome a ser analisado.
...
//------------------------BOTOES-------------------------
	/**
	 * Essa função é responsavel por gerenciar o clique do botão de confirmação de
	 * login e cadastro de conta. Além disso, a função MouseCliked é chamada e ela
	 * executa 2 funções a de criar conta e fazer login e o que irá diferenciar se o
	 * usuario está logando ou criando conta é o boolean "Logando", isso é util para
	 * o mesmo botao ter 2 funcionalidades diferentes.
	 */
		...
		telaLogin.getConfirmacao().addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (telaLogin.getLogando())
					fazerLogin(telaLogin.getNome(), telaLogin.getSenha(), telaLogin.getTipo());
				else
					criarConta(telaLogin.getNome(), telaLogin.getSenha(), telaLogin.getNovaSenha(),
							telaLogin.getTipo());
			}
		});
	}
