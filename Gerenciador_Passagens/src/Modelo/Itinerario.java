...
/**
 * A classe itinerario teŕa todas as informações que compoẽ um itinerário e um
 * id único.
 * 
 * @author joaoseisei
 * @since 2023
...
//CONSTRUTOR
	...
	/**
	 * O construtor de Iterário recebe todos os atributos necessários para a criação
	 * do mesmo, pois é importante não criar um objeto dele com informações
	 * pendentes, também existe uma verificação para impedir datas iniciais
	 * posteriores as finais. Além disso é randomizado um id aleatório para todo
	 * itinerário criado.
	 * 
	 * @param dataInicial
	 * @param dataFinal
	...
	public Itinerario(LocalDate dataInicial, LocalDate dataFinal, LocalTime horaInicial, LocalTime horaFinal,
		...
		// DATA FINAL
		if (dataFinal.isAfter(dataInicial) || dataFinal.equals(dataInicial))
			this.dataFinal = dataFinal;
		else
			throw new IllegalArgumentException("Data final é anterior a inicial");
		// HORA INICIAL
		this.horaInicial = horaInicial;
		// HORA FINAL
		if (dataFinal.isAfter(dataInicial) || dataFinal.equals(dataInicial) && horaFinal.isAfter(horaInicial))
