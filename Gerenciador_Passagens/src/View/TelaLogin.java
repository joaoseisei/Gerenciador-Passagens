...
/**
 * A classe TelaLogin é responsavel por exibir todo o painel de login utilizando
...
//ATRIBUTOS
	private JFrame tela;
	private JPanel painel, container, conteudoContainer, controle, conteudoControle, registro, conteudoRegistro, login,
			conteudoLogin, confirmacao, conteudoConfirmacao;
...
//CONSTRUTOR
	/**
	 * O contrutor de TelaLogin não recebe parametros, ele basicamente só coloca
	 * todo o conteudo da interface grafica quando a classe é criada para sempre que
	...
	public TelaLogin() {
		...
		// Na funcao arredondar bordas eu chamo 2 paneis, um servirá como borda
		// arredondada,
		// e o outro será o painel principal com conteudos, no caso o conteudo container
		// é onde colocarei
		// todos os inputs, e por ultimo recebe como parametro o raio da borda
		// arredondada
		...
		// Basicamente eu to pegando um textfield comum e passando a JSSTEXTFIELD para
		...
		tipoSeletor.addMouseListener(new MouseAdapter() {
...
//SELECIONAR TIPO
	/**
	 * Esse método é responsável por definir o tipo de conta que irá se registrar
	 * (Usuario ou Admin), além disso ele define a cor para saber qual está
	 * selecionado
	 */
...
//REGISTRO
	/**
	 * Esse método é responsável por movimentar a tela para a área de registro, e
	 * define o logando como false, para saber que o usuário está registrando, além
	 * disso cria uma nova Thread para fazer a animação de movimentação da dela e
	 * adiciona um novo JTextField para confirmação de senha.
	 */
	public void registro() {
		// ANIMACAO
		new Thread(() -> {
			try {
				Animator.moverHorizontal(container, 1, 95, 305, 2);
			} catch (InterruptedException y) {
			}
			// É necessario criar um Thread para a animacao funcionar corretamente, pois ele
			// irá desencadear o loop que existe um Thread.sleep para deixar a animacao mais
			// suave
		}).start();
		// FUNCOES
...
//LOGIN
	/**
	 * Esse método é responsável por movimentar a tela para a área de login, e
	 * define o logando como true, para saber que o usuário está logando, além disso
	 * cria uma nova Thread para fazer a animação de movimentação da dela.
	 */
	public void login() {
		// ANIMACAO
		new Thread(() -> {
			try {
				Animator.moverHorizontal(container, 1, 405, 235, -2);
			} catch (InterruptedException y) {
			}
			// É necessario criar um Thread para a animacao funcionar corretamente, pois ele
			// irá desencadear o loop que existe um Thread.sleep para deixar a animacao mais
