...
/**
 * A classe TelaUsuario é responsavel por exibir todo o painel de login
 * utilizando o swing do java, em forma de interface gráfica.
 * 
 * @author joaoseisei
 * @since 2023
 * @version 1.2
...
//ATRIBUTOS
	private JFrame tela;
	private JPanel painel, configuracao, conteudoConfiguracao, filtro, conteudoFiltro, passagem, icone, od, oe, testa,
			sair, info, utilidade, fav, confirmacao;
	private JScrollPane container;
	private JLabel userLB, sairLB, infoLB, subtituloLB, itinerarioLB, passagemAviaoLB, passagemOnibusLB, favLB,
			confirmacaoLB, dataInicialLB, dataFinalLB, pontPartidaLB, pontChegadaLB;
...
//CONSTRUTOR
	/**
	 * O contrutor de TelaUsuario recebe como parametro um Usuário, isso é para que
	 * cada TelaUsuario se adapte ao usuário, além disso basicamente é colocado todo
	 * o conteudo da interface gráfica quando a classe é criada para sempre que
	...
	public TelaUsuario(UsuarioControle usuario) {
		...
		icone.setBorder(BorderFactory.createStrokeBorder(
				new BasicStroke(117, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND), Color.DARK_GRAY));
		...
		od.setOpaque(false);
		od.setBorder(BorderFactory
				.createStrokeBorder(new BasicStroke(44, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND), Color.white));
		icone.add(od);

		oe = new JPanel();
		oe.setBounds(10, 35, 45, 45);
		oe.setOpaque(false);
		oe.setBorder(BorderFactory
				.createStrokeBorder(new BasicStroke(44, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND), Color.white));
		icone.add(oe);
		...
		fav.setOpaque(false);
		fav.setBorder(BorderFactory
				.createStrokeBorder(new BasicStroke(25, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND), Color.GRAY));
		...
		info.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				JOptionPane.showMessageDialog(null, "Obrigado por acessaro o gerenciador de passagens papo\n"
						...
						+ "i para itinerário, a para avião e o para ônibus", "INFORMAÇÕES",
		...
		sair.setBorder(BorderFactory.createStrokeBorder(
				new BasicStroke(25, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND), new Color(145, 84, 234)));
		...
		confirmacao.setBorder(BorderFactory.createStrokeBorder(
				new BasicStroke(25, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND), new Color(145, 84, 234)));
		...
		container.setBorder(BorderFactory
				.createStrokeBorder(new BasicStroke(5, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND), Color.BLACK));
		...
		itinerarioLB.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				trocarItinerario();
			}
		});
		...
		passagemAviaoLB.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				trocarPassagemAviao();
			}
		});
		...
		passagemOnibusLB.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				trocarPassagemOnibus();
			}
		});
...
//METODOS
	/**
	 * Esse método é responsável por adicionar passagens e itinerarios no container,
	 * além disso ele também adiciona um botão de adicionar nos favoritos do usuario
	 * e verifica se essa passagem ja existe nos favoritos, se não existir ou a
	 * lista de favoritos estiver vazia ele adiciona, também é feito um separamento
	 * de linha, para que o container tenham diversas linhas uma em baixo da outra
	 * utilizando o .slipt("\n"), pois o próprio tostring ja tem o tamanho ideal e
	 * barramento necessário.
	 *
	 ...
	 * adicionar uma label com a mesagem "resultado:", isso é utilizado para resetar
	 * o container sempre que for filtrar algo.
	 * 
	 * @param container
	 * @param texto
	 */
	...
	/**
	 * Esse método é responsavel por definir o tipo de itinerario que o usuário
	 * deseja ver, se for true ele irá ver, e se for false a letra ficará vermelha e
	 * o usuário não verá os itinerarios.
	 */
	...
	/**
	 * Esse método é responsavel por definir o tipo de passagem que o usuário deseja
	 * ver, se for true ele irá ver, e se for false a letra ficará vermelha e o
	 * usuário não verá as passagens de avião.
	 */
	 ...
	 * Esse método é responsavel por definir o tipo de passagem que o usuário deseja
	 * ver, se for true ele irá ver, e se for false a letra ficará vermelha e o
	 * usuário não verá as passagem de ônibus.
	 */
